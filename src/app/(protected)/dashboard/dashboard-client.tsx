...
import { canModifyForDate, isOpenDay } from "@/lib/settings-utils";
...
export function DashboardClient({
  ...
  const currentWeekStart = useMemo(
    () => startOfWeek(new Date(weekStartISO), { weekStartsOn: 1 }),
    [weekStartISO]
  );
  ...
  const reservationsByDate = useMemo(() => {
    ...
    reservations.forEach((r) => map.set(new Date(r.date).toDateString(), r));
    return map;
  }, [reservations]);

  const getMenuForDate = (date: Date) => menusByDate.get(date.toDateString());
  const getReservationForDate = (date: Date) => reservationsByDate.get(date.toDateString());

  const handleDayClick = (date: Date) => {
    const menu = getMenuForDate(date);
    const reservation = getReservationForDate(date);
    const status = canModifyForDate(date, settings.reservationDeadline);

    if (!status.canModify || !menu) return;
  ...
  const navigateWeek = (delta: number) => {
    const next = addWeeks(currentWeekStart, delta);
    const param = format(next, "yyyy-MM-dd");
    startTransition(() => {
      router.push(`/dashboard?week=${param}`);
    });
  };
    ...
    <>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-rasf-900">
              Heure limite de réservation : <span className="font-bold">{settings.reservationDeadline}</span>
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {weekDays.map((day) => {
          ...
          const status = canModifyForDate(day, settings.reservationDeadline);
          const open = isOpenDay(day, settings.openDays);
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>Après {settings.reservationDeadline}</span>
          </div>
        </div>
      </div>
      ...
      {selectedDate && selectedMenu && (
        <ReservationModal
          ...
          userId={userId}
          deadlineTime={settings.reservationDeadline}
        />
      )}
    </>
  );
